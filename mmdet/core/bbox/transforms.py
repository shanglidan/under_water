...
def bbox_vflip(bboxes, img_shape):
    """Flip bboxes horizontally.
...
def bbox_mapping(bboxes, img_shape, scale_factor, flip, vflip):
    """Map bboxes from the original image scale to testing scale"""
    new_bboxes = bboxes * scale_factor
    if flip:
        new_bboxes = bbox_flip(new_bboxes, img_shape)
    if vflip:
        new_bboxes = bbox_vflip(new_bboxes, img_shape)
    return new_bboxes


def bbox_mapping_back(bboxes, img_shape, scale_factor, flip, vflip):
    """Map bboxes from testing scale to original image scale"""
    new_bboxes = bboxes
    if flip:
        new_bboxes = bbox_flip(bboxes, img_shape) 
    if vflip:
        new_bboxes = bbox_vflip(bboxes, img_shape) 
