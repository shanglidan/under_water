...
class ColorJitter(object):
    """EqualizeHist the image

    if the input dict contains the key "equalize", then the flag will be used,
    otherwise it will be randomly decided by a ratio specified in the init
    method.

   Args:
       equalize_ratio (float, optional): the equalizing probability

    """

    def __init__(self, ratio=None, transform_dict=None):
        self.ratio = ratio
        self.transform_dict = transform_dict
        transform_type_dict = dict(
             brightness=PIL.ImageEnhance.Brightness, contrast=PIL.ImageEnhance.Contrast,
             sharpness=PIL.ImageEnhance.Sharpness,   color=PIL.ImageEnhance.Color)
        self.transforms = [(transform_type_dict[k], transform_dict[k]) for k in transform_dict]
        if ratio is not None:
            assert ratio >= 0 and ratio <= 1

    def __call__(self, results):
        if 'colorjitter' not in results:
            colorjitter = True if np.random.rand() < self.ratio else False
            results['colorjitter'] = colorjitter
        if 'rand_num' not in results:
            rand_num = np.random.uniform(0, 1, len(self.transforms))
            results['rand_num'] = rand_num
        if results['colorjitter']:
            img = PIL.Image.fromarray(cv2.cvtColor(results['img'],cv2.COLOR_BGR2RGB))
            for i, (transformer, alpha) in enumerate(self.transforms):
                r = alpha * (rand_num[i]*2.0 - 1.0) + 1   # r in [1-alpha, 1+alpha)
                img = transformer(img).enhance(r)
            results['img'] = cv2.cvtColor(np.asarray(img),cv2.COLOR_RGB2BGR)
        return results
    def __repr__(self):
        return self.__class__.__name__ + str(self.transform_dict)
...
class FixedPad(object):
    ...
    def __init__(self, x_scope=None, y_scope=None):
        self.x_scope = x_scope
        self.y_scope = y_scope

    def bbox_pad(self, bboxes, img_shape):
        """Flip bboxes horizontally.
        ...
        assert bboxes.shape[-1] % 4 == 0
        flipped = bboxes.copy()
        if self.x_scope is not None:
            flipped[..., 0::4] = bboxes[..., 0::4] - self.x_scope[0]
            flipped[..., 2::4] = bboxes[..., 2::4] - self.x_scope[0]
        if self.y_scope is not None:
            flipped[..., 1::4] = bboxes[..., 1::4] - self.y_scope[0]
            flipped[..., 3::4] = bboxes[..., 3::4] - self.y_scope[0]
        return flipped

    def __call__(self, results):
        img = results['img']
        if self.x_scope is not None:
            img = img[:, self.x_scope[0]:self.x_scope[1], :]
        if self.y_scope is not None:
            img = img[self.y_scope[0]:self.y_scope[1], :, :]
        results['img'] = img
        # crop bboxes
        for key in results.get('bbox_fields', []):
            results[key] = self.bbox_pad(results[key],
                                          results['img_shape'])
        return results
...
class Equalize(object):
   ...
   Args:
    ...
    def __init__(self, equalize_ratio=None):
        self.equalize_ratio = equalize_ratio
        if equalize_ratio is not None:
            assert equalize_ratio >= 0 and equalize_ratio <= 1

    def __call__(self, results):
        if 'equalize' not in results:
            equalize = True if np.random.rand() < self.equalize_ratio else False
            results['equalize'] = equalize
        if results['equalize']:
            (b, g, r) = cv2.split(results['img'])
            b = cv2.equalizeHist(b)
            g = cv2.equalizeHist(g)
            r = cv2.equalizeHist(r)
            results['img'] = cv2.merge((b, g, r))
        return results
    def __repr__(self):
        return self.__class__.__name__ + '(equalize_ratio={})'.format(
            self.equalize_ratio)
...
class Brightness(object):
   ...
   Args:
    ...
    def __init__(self, brightness_ratio=None, v=0.64):
    ...
    def __call__(self, results):
        if 'brightness' not in results:
            brightness = True if np.random.rand() < self.ratio else False
            results['brightness'] = brightness
        if results['brightness']:
            img = PIL.Image.fromarray(cv2.cvtColor(results['img'],cv2.COLOR_BGR2RGB))
            img = PIL.ImageEnhance.Brightness(img).enhance(self.v)
            results['img'] = cv2.cvtColor(np.asarray(img),cv2.COLOR_RGB2BGR)
        return results
    def __repr__(self):
        return self.__class__.__name__ + '(ratio={})'.format(
            self.ratio)
...
class BBoxJitter(object):
    ...
    def __init__(self, min=0, max=2, ratio=None):
        self.ratio = ratio
        self.min_scale = min
        self.max_scale = max
        self.count = 0
    ...
    def __call__(self, results):
        if 'bboxjitter' not in results:
            bboxjitter = True if np.random.rand() < self.ratio else False
            results['bboxjitter'] = bboxjitter
        if results['bboxjitter']:
            for key in results.get('bbox_fields', []):
